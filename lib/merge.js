  ...
  if (context.config.mergeDeleteBranch) {
    try {
      await deleteBranch(
        octokit,
        pullRequest,
        context.config.mergeProtectBranches
      );
    } catch (e) {
...
async function deleteBranch(octokit, pullRequest, mergeProtectBranches) {
  ...
  if (branch.protected || mergeProtectBranches.includes(branch.name)) {
