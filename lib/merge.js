...
async function merge(context, pullRequest) {
  ...
  if (context.config.mergeDeleteBranch) {
    try {
      await deleteBranch(octokit, pullRequest, context.config.protectBranches);
    } catch (e) {
...
async function deleteBranch(octokit, pullRequest, protectBranches) {
  ...
  if (branch.protected || protectBranches.includes(branch.name)) {
