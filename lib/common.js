...
function createConfig(env = {}) {
  ...
  const mergeRemoveLabels = parseCommaSeparatedList(
    env.MERGE_REMOVE_LABELS,
    ""
  );
  ...
  const mergeDeleteBranchFilter = parseCommaSeparatedList(
    env.MERGE_DELETE_BRANCH_FILTER,
    ""
  );
  const mergeMethodLabels = parseCommaSeparatedList(env.MERGE_METHOD_LABELS);
